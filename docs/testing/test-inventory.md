...
empty list. There are forty-five, and they are listed in their own section rather than
hidden in the data.
...
Forty-three modules. Committed machine-readable data checked against the documents
...
to the strategy's, every evidence record held to the evidence-level rules, and the
README's public entry points held to the rows that govern them. Then came the proof
dashboard generated from that register: the committed page regenerated and compared,
every count on it recomputed, and each rule its generator applies driven over a
register corrupted to break it. The newest is the evidence migration: the migrated
register compared with the superseded one it was read from, every change it made named
in a committed audit record, and every number its report publishes recomputed.
...
records they summarise, `V1-S5-011-PR1` the thirty-fifth, which holds the
evidence-level specification to the documents it supersedes, and `V1-S5-011-PR2`
the thirty-sixth, which holds the versioned evidence-record model to the shape
that specification describes, `V1-S5-012-PR1` the thirty-seventh, which drives
every enforced evidence-level classification rule over a register corrupted to
...
`V1-S5-007-PR1` the forty-first, which holds the case study to the register, the
completeness ledger, and the records whose figures it quotes, `V1-S5-013-PR1` the
forty-second, which holds the closure of the five release blockers to the ledgers, the
reruns' transcripts, and every surface that publishes the claims, and `V1-S5-013-PR2`
the forty-third, which holds the case study's publication and the evidence freeze to
the publication ledger, the dated records it corrects, the repository's history, and
...
Forty-five suites protect something no row of the claim matrix names. (This sentence
...
and corrected it, `V1-S5-006-PR1` added the forty-first, `V1-S5-006-PR2` the forty-second, `V1-S5-007-PR1` the forty-third, `V1-S5-013-PR1` the forty-fourth, and `V1-S5-013-PR2` the forty-fifth. The machine-checked count is the one in the opening section.) Each carries its
...
| [`tests/testing/test_evidence_levels.py`](../../tests/testing/test_evidence_levels.py) | The InferOps evidence-level model against the documents that publish it: that [the specification](evidence-levels.md) is the only document binding `C0` to `C4` to their current names, that a superseded meaning survives only in a mapping document or a dated record and carries a supersession notice, that no surface is left awaiting migration, that the legacy mapping and the project-defined disclaimer are published, that ADR 0005 D4's accepted text is annotated rather than rewritten, and that the enforcing strategy data carries the current names and no longer lets the `synthetic` class cover generated input. That last check was a tripwire that failed when the data moved, so that the page describing the old state would be corrected in the same change. (This row named `V1-S5-011-PR2` as that moment until 2026-09-23. It was wrong: that change versioned the evidence-record model and deliberately left the strategy data alone, so the tripwire did not fire. The row then named `V1-S5-012-PR1`, and that was wrong too: that change published the rules that replace the ceiling for `v1alpha2` records and deliberately left the strategy data governing the committed `v1alpha1` register. The moment was `V1-S5-012-PR2`, which migrated both, and the tripwire fired there and was replaced by the check above.) Deliberately no claim: it establishes that the repository states one definition, never that the definition is a good one or that any record is classified correctly under it |
| [`tests/testing/test_evidence_record_model.py`](../../tests/testing/test_evidence_record_model.py) | The versioned claim and evidence data model against what the evidence-level specification says a level is: that [the `v1alpha2` schema](claim-evidence-matrix.v1alpha2.schema.json) puts the level on an evidence record rather than on a claim, that one claim may hold several records at several levels, that a valid shape exists for each of `C0` to `C4` including the two this repository has never reached, that substitution and workload origin are separate fields so generated input imposes no ceiling, that every shape the model exists to refuse is refused where the committed refusal matrix says it is, that the superseded `v1alpha1` register still reads into the new version with every field accounted for, nothing given an evidence level, and every superseded classification carried verbatim, and that the authoritative register declares `v1alpha2` and holds its levels on records. Its two tripwires fired when `V1-S5-012-PR2` migrated the register, and were replaced by those last checks. Deliberately no claim: it establishes that a shape exists and is consistent, never that any evidence is classified correctly under it |
...
| [`tests/testing/test_evidence_closure.py`](../../tests/testing/test_evidence_closure.py) | The closure of the V1 evidence blockers against what it rests on: that every blocker the completeness check raised has exactly one disposition by one of three mechanisms, and a closure that drops, doubles, or invents one is refused; that the gate is the one the closure decides and fails on an open blocker; that every rerun's transcript shows the named revision and an empty status on both sides, and every added record names that revision and quotes it verbatim; that no older record was edited, removed, or given another level, and only a claim decision moved a status, downwards; that every closed claim says how where the dashboard publishes it; and that the closure report's counts, gate output, and freeze-candidate digest are what the ledger and the index produce. Deliberately no claim: consistency, not correctness |
| [`tests/testing/test_evidence_publication.py`](../../tests/testing/test_evidence_publication.py) | The case study's publication and the V1 evidence freeze against what they rest on: that every register change the publication ledger names is to a field a publication may change, answers a finding, and moves no status, statement, level, or record; that every dated record the case study's verification carried has a correction beside it and is unchanged; that the two readings the corrections rest on are re-derived from the committed recovery record and telemetry; that the freeze is refused beside an open blocker; that the evidence pack covers the cited files, the register, and every ledger, and a changed register moves it and not the set digest; that the superseded candidate recomputes from the base revision; that the gate prints the freeze and refuses a stale index; that no current surface describes publication as still to come; and that the report states what the ledger and the index produce. Deliberately no claim: never that a correction is the right reading, or that the case study's prose says what its records say |
| [`tests/testing/test_case_study.py`](../../tests/testing/test_case_study.py) | The V1 engineering case study, published, against what it cites: that every claim it names is a register row and each section cites exactly the claims [its data file](../case-study/v1-engineering-case-study.v1alpha1.json) declares; that every cell of its claims appendix is derived from the register and the completeness and closure ledgers; that its section on what V1 does not prove cites every uncertified claim and every release blocker; that every figure it quotes reads back from the committed file it names and no measurement appears without a declared source; that every count it states is recomputed; that it names the evidence set and the evidence pack it was verified against and fails when either digest moves; that its results-first summary comes first, stays short, rests on no release blocker, and links each finding to its section; that every figure on the page is declared, every numeral and count word in a text figure's body is declared, and every caption states its boundary; that no reader surface says nobody intervened in the pod-loss experiment; that it quotes no amount from a cost result; and that it is a draft exactly while the evidence is not frozen, published exactly once it is, and then linked from the README as an entry point the register lists as claiming nothing. Deliberately no claim: agreement with the files it quotes, never that a sentence or a figure's arrangement says what its record says |
