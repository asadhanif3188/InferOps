...
| Date accepted | 2026-09-15, for D1, D2, D4, D5, D6, and D7; D3 is accepted as a rule |
| Decision owner | Unassigned; no public maintainer roster exists yet |
| Supersedes | None |
| Amends | [ADR 0007](ADR-0007-inference-cost-method.md) D1, D2, and D9, and the committed method data that carries them |
| Superseded by | None |
...
> `V1-S4-004`, the node's cgroup processor counters and memory readings for a
...
> selected (D12), and which platform component computes and emits a cost record in a
> running system stays undecided (D13). The tool runs by hand, contacts nothing, and
> emits no telemetry. Every usage value it reads is **typed in by hand**; nothing in
...
| D3 | What counts as measured use, and how a usage value is taken from samples | **Accepted as a rule.** The evidence-class half is tested; the integration half is review only, because no reader exists | A test that synthetic usage never derives `hasMeasuredUtilisation`, and nothing for the integration rules |
| D4 | The estimated amount, its unallocated line, and what closes | **Accepted** | Fixtures whose figures are held to values worked out by hand, and a closure check the tool runs on every result |
...
| D7 | A repository tool computes a record by hand; nothing else about ownership or publication moves | **Accepted** as a scope rule | Review, and the method's `computationStatus`, which a test reads |
...
`estimated` anyway, "so that the first component to measure utilisation produces this
basis deliberately rather than relabelling an allocation". D2 likewise deferred
`observed-utilisation-share` "so that acquiring a metrics server means deleting a
deferral rather than inventing a method".

Two things have happened since. `V1-S4-004` ran a declared scenario matrix on
`docker-desktop` and committed, beside it, the node's cgroup processor counters and
memory working-set readings for every release pod, sampled every few seconds on one
clock. And `V1-S4-005` asks for an hourly figure and a cost-per-request figure on the
estimate basis, validated against the shape the method publishes.
...
produces and which is not edited by this record.
...
The honest cost is the one ADR 0007 already wrote for observed use: it rewards a
workload for reserving capacity it does not use. Under this basis that idle
...
other class may supply usage. Only a measured class, with processor and memory use both
present, sets a record's `hasMeasuredUtilisation` fact. A synthetic input must say it is
synthetic in its own warning.

A usage value is taken from committed samples of a declared experiment that qualifies
under ADR 0013 D3, as follows:

- **processor seconds** are the increase of the container's cumulative cgroup
  processor counter between the samples that bound the window, summed over the
  workload's pods. A counter that resets inside the window makes the value unavailable
  with the reason `input-conflict`; no reset is stitched;
- **memory byte-seconds** are the container's working-set bytes integrated
...
**Enforcement.** The evidence-class rule is enforced by the tool and tested. The
integration rules above are **review only**: no reader in this repository takes a
usage value from the samples. Every usage value in an input today is typed in by hand,
and the tool can check that it is shaped correctly and fits within the node's declared
capacity for the window, but cannot check that it matches the evidence it names. That
...
close against the node exactly. If any workload amount is null, the unallocated line is
...
Measured use that exceeds what the node's declared capacity holds in the window, for one
workload or all of them together, is refused as conflicting inputs. So is a device
second reported for a workload that reserves no device. A prerequisite resource keeps
...
no rounding. Its share is divided from those same published figures.
...
window** and projects nothing beyond it; ADR 0007 D4's window rules are unchanged, so a
window is at least a minute and at most a day.
...
That is not an answer to ADR 0007 D13. Which platform component computes and emits a
cost record in a running system, and who owns it, stays not decided. ADR 0007 D10 is
...
D3's reported residual, D4's windows, D5's committed and synthetic-only prices, D6's
decimal money and binary memory, D7's null with a reason, D8's derived confidence, D10,
D11, D12, and D13 are unchanged. The text of ADR 0007 is kept as it was accepted, with a
pointer to this record beside D1, D2, and D9.
...
`publishedCostFigures`, the record shape gains four fields, and a record's amount
becomes nullable. The worked example and its figures are unchanged.
...
value shaped like a host path, a user directory, or a network address. It reads a
