...
| R6 | The installed WSL release and kernel are substantially older than the operating-system build they run on | Open | The cluster works on it today. The engine reports cgroup v1, which the Kubernetes tool warns is deprecated and will stop supporting, so this is a dated dependency rather than a stable one. |
