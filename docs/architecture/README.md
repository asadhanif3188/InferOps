...
| [0012](decisions/ADR-0012-continuous-integration-service.md) | Continuous-integration service for the default lane | Accepted in part | 2026-09-12 | [Change validation](../proof/testing/v1-s4-001-pr1-validation.md); one committed workflow runs the `default-checks` lane and a gate matrix is compared to it in both directions — and no job in it has executed on the service, so the file is a configuration rather than a result. ADR 0005 D6's capable-runner half stays open |
