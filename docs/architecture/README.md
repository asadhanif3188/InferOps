...
alert routing path are explicitly not assigned, and since the 2026-09-13 and
2026-09-17 amendments the dashboard definition and the alert definition are
repository artifacts. In 0005, five decisions are accepted and
...
passing; 0004 has since assigned the collector, the dashboard
...
`D7` was amended on 2026-09-09 to make it Helm-owned and release-scoped, on
2026-09-13 to make the dashboard definition a repository artifact, and on 2026-09-17
to make the alert definition one as well; a durable store, a dashboard server and
alert routing stay deferred as `telemetry-backend`.
...
store, dashboard server, or alert routing path exists. A dashboard definition does,
checked against the query policy and validated once in a throwaway Grafana that
nothing owns, and so do six alert definitions, checked against the same policy and
driven across eight scenarios by nothing that has ever notified anybody. The selected serving runtime's own signals
...
invoice has ever been read, and no platform component computes a cost record. (Since
2026-09-15, [0014](decisions/ADR-0014-v1-cost-calculation-reaches-the-estimated-basis.md) makes `estimated` the only basis V1 calculates
and adds a repository tool that computes a record by hand; every figure it can produce
is still `none`.) It also records
...
throughput figure is also a rule against publishing a cost-per-request figure. (0013
has since narrowed the throughput rule to bounded local observations; 0007 D11 still
publishes no cost figure.) Two of
