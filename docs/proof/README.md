...
cited file bound to its content by SHA-256. It is generated by
`python -m tools.evidence_index` from the register and
[the normalization ledger](testing/v1-s5-006-pr1-normalization.v1alpha1.json), and a
test regenerates it. Where a record here was found to say something its own evidence
contradicts, the correction is in that ledger, beside the record, and the record is
