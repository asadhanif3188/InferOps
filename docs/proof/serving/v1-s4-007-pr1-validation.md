...
alternatives.** Five mechanisms were considered and four of them were measured against
the pinned image and the cached artifact before this one was chosen. The descriptor
lists each with the reason it was rejected, and the tool refuses a descriptor that names
fewer than three — because a descriptor stating only the mechanism that was taken hides
the argument. In short: an artifact size or hash mismatch is `V1-S3-008`'s and is
refused before the runtime container starts; an absent artifact fails the same init
container, which reads the same derived path the runtime is given, by design; a
well-sized file that is not a GGUF makes `llama-server` **exit 1**, which is restart
churn and not a healthy process; a context size at the schema maximum renders in
scientific notation from a values file, which the API refuses at start-up and which
`llama-server` reads as a context of `1` and aborts on; and a health path the runtime
does not serve would leave the model ready and the probe wrong.
...
uv run --locked python -m pytest tests/architecture -q             2258 passed, 3 skipped
uv run --locked python -m pytest tests/testing -q                  1863 passed
uv run --locked python -m pytest tests/security -q                 832 passed
uv run --locked python -m pytest -q                                9560 passed, 30 skipped, 14 deselected
...
The offline suite is 107 tests and sits in the default lane, because it contacts
...
| The direct `docker run` measurements that chose the mechanism | **local real**, recorded as prose in the descriptor and the overlay rather than as a record | descriptor `disruption.rejectedMechanisms` |
...
| Liveness does not destructively restart the runtime | **Met, and bounded.** Restart count 0 at every sample; the socket opened 20 272 ms after the container started and answered throughout. The window is 30.0% of the runtime's own startup probe budget, and the record publishes that share so the bound travels with the figure |
