...
| Component and resource-ownership boundaries | Component decomposition, dependency direction, and the Terraform/Helm/controller ownership split | [ADR 0004](../architecture/decisions/ADR-0004-component-and-ownership-boundaries.md) accepted in part; the ownership inventory is machine-checked, and most components it describes are now built and were observed in a cluster on the `docker-desktop` reference provider |
| Telemetry collection and ingress ownership | Collector selected, dashboard and alert definitions owned; the rest not | [ADR 0004](../architecture/decisions/ADR-0004-component-and-ownership-boundaries.md) D7, amended 2026-09-09: the collector is Helm-owned and release-scoped and a release has installed one; amended 2026-09-13: the dashboard definition is a repository artifact, validated once in a throwaway Grafana and served by nothing; amended 2026-09-17: the alert definition is a repository artifact, checked against eight scenarios and evaluated by nothing. An ingress or load-balancer implementation, a durable store, a dashboard server and alert routing stay unowned and deferred out of V1 |
