...
- **The V1 case study verified against the evidence as it stands, and kept a draft.**
  ...
  `python -m tools.evidence_index --gate` still exits 1 on five release blockers — so
  the page stays a **draft**, the README still does not link to it, and the change
  says so rather than publishing around the gate. What it did instead: it checked the
  page against the evidence set whose digest the index states, and a test now fails
...
- **A release gate for the V1 evidence, and it is not passed.** `V1-S5-006-PR2` checked
  ...
  state — 11 resolved, 10 narrowed, 4 historical corrections, 2 blockers, none
  downgraded — and reads how each of the 34 executed records identifies the repository
  code that ran. [The completeness report](docs/proof/testing/v1-s5-006-pr2-evidence-completeness.md)
  explains each. **Five certified claims are release blockers**: the local serving
  baseline, the kind cluster helper, the Helm uninstall's survival clause, the upgrade
  rollback, and the pod replacement each rest on a record whose repository code nothing
  identifies, and each claim's limitation now says so. Every blocker names the run that
  would close it and the decision that could replace the run. `python -m tools.evidence_index --gate`
  prints the decision and exits 1 while any blocker stands, so the evidence pack is
  **not frozen**.

- **Three records at a named revision.** The domain, API, and API-telemetry suites were
  run again in a clean detached worktree at the merge of `V1-S5-006-PR1`, and
  [the run](docs/proof/testing/v1-s5-006-pr2-pinned-suite-run.md) is a new record under
  each of the three claims whose only record named a branch point rather than the tree
  that ran. The older records stay cited for what they did.

- **The evidence index is the evidence manifest.** Each cited file now carries its git
  blob name beside its SHA-256, and a test checks every one against what git holds; the
  whole set has one `evidenceSetSha256` a release can quote; each executed record carries
  its code identity; each claim carries its release blockers. The index now reads 60
  records under 59 claims, and 16 of the 34 executed records name the revision that ran.

- **The V1 evidence index.** [`v1-evidence-index.v1alpha1.json`](docs/proof/v1-evidence-index.v1alpha1.json),
  generated by `python -m tools.evidence_index`, holds one entry per evidence record:
  what executed and was substituted, the workload source, the environment and provider,
  every immutable identifier the record pins, the repository revision it names and how
  that revision relates to what ran, and every cited file with a SHA-256 of its
  committed content. A test regenerates it and fails on any difference, and fails if a
  cited file is untracked, outside `docs/proof/`, or changed since it was indexed.
  [Its page](docs/proof/v1-evidence-index.md) says what it holds and what it does not.
  It shows that 13 of the 31 records that executed their target behaviour name the
  revision that ran.

### Changed

- **Four experiment records cite the file that pins their code.** The real-load,
  performance-matrix, pod-loss, and unready-model runs each started from a named commit
  with their change's files uncommitted, and each wrote the LF-normalised SHA-256 of
  every file that decided it into an environment file none of them cited. They cite it
  now, and a test checks each of the 34 hashes against the commit that first carried
  the file.

- **A current data file no longer pairs `C3` with its superseded meaning.** The strategy
  data's failure-and-resilience note said full `C3` failure certification was out of
  scope; it now names no level, and a test reads every current JSON data file for the
  superseded pairings, which the Markdown guard could not see.

- **Every finding the evidence migration left open has a disposition.**
  [The normalization ledger](docs/proof/testing/v1-s5-006-pr1-normalization.v1alpha1.json)
  answers the migration audit's 17 open items and 4 findings of its own, each with one
  of five dispositions, and names all 25 register changes with the value before and
  after; [the report](docs/proof/testing/v1-s5-006-pr1-evidence-normalization.md)
  explains each. Four claim statements were **narrowed** to what their records support:
  the eleven gates no longer claim a hosted result no record promotes, the pod-loss run
  no longer says no person intervened, the local baseline says its fixture was rewritten
  after the registered one was refused, and the troubleshooting guide excepts the Linux
  probe it never ran. Three records were **added** where a committed run supported part
  of a claim no cited record did — the link suite's own run, the paved-road install
  behind the Helm claim, and the clean-clone run's committed per-query telemetry — and
  the manifest-refusal record stopped citing a run that reads no pod, which also exposed
  that it pinned another change's chart version. No status moved, no existing record's
  level moved, and nothing was promoted. The register now holds 57 evidence records.

- **Nine historical records carry a correction beside them, and none was rewritten.**
  The ledger stores each record's content hash, and a test recomputes it. Among them: a
  baseline run whose stated revision the repository's history dates after the run, a
  telemetry result whose `verified` does not check the absence query that returned an
  unexplained sample, and two runs that name the model differently because the name is
  an operator-set label.

- **The migration suite compares the migration, not everything since.** It undoes every
  change the ledger names before comparing the register with `v1alpha1`, so a register
  change the ledger does not name fails it.
...
- [The evidence-record model](docs/testing/evidence-record-model.md) said the proof
  ...
  [the architecture index](docs/architecture/README.md) said ADR 0016's rules were not
  yet enforced and migration was still to come. Both describe the migrated state now,
  and a test refuses those sentences in any current document.
- The README and [the pod recovery page](docs/serving/inference-pod-recovery.md) said no
  person intervened in the pod-loss run; the record establishes only that the workflow
  issued no mutating command, and both now say that.
...
- **The V1 security and observability methods are published, and each keeps what is
  ...
  secrets, API and network exposure, container and Kubernetes controls, scan and
  policy gates, accepted exceptions, prompt and response handling, deferred risks, and
  the distance between production-oriented design and production certification.
  [The observability method](docs/telemetry/observability-method.md) walks nine:
  resource and AI attributes, logs and redaction, metrics and cardinality, collection,
  dashboard semantics, alerts, retention assumptions, deferred gaps, and the same
  distance. Every topic has two tables. An implemented row names a test or a gate and a
  committed record; a row that is not implemented names the register entry, uncertified
  claim, unemitted metric, or recorded gap that carries it, and what may not be claimed.

  **The line between the two tables is not drawn by the author.**
  [`tests/testing/test_published_methods.py`](tests/testing/test_published_methods.py)
  reads the security baseline, the telemetry catalog, the alert record, the claim
  register, and the workflow, and puts every control on the side its own derived status
  allows and every metric on the side its emission field allows; it refuses an
  implemented row resting on an uncertified claim and a gap resting on a certified one,
  resolves every test function, gate, and record named, requires every control, register
  entry, exception, metric, and alert to appear, and holds each document's sections to
  exactly the identifiers its record holds. Neither method adds a control, a signal, or a
  ...
  **Writing them found five kinds of statement that had stopped being true**, and each
  ...
  matrix's own statement that its two infrastructure gates have never run on the
  service, which the same run list shows passing eighteen times.
...
- **Every V1 claim, and the eight this project will not make.**
  ...
  **Ten rules, each naming the control that has been watched refusing it.** A row mapped
  to a claim in the test strategy may be more conservative than that claim and never
  bolder. A certified row asserting real serving, performance, or reliability behaviour
  must rest on an evidence label whose class may support one, so a mock can never appear
  behind a serving sentence. A level may not exceed its label's ceiling. A real
  Kubernetes row names a provider the cluster provider contract publishes, so Docker
  Desktop evidence cannot be read as `kind`. A row whose claim the test inventory records
  as covered by no pytest module carries that gap rather than letting the modules it
  names imply coverage. And no row describes an amount in the vocabulary reserved for an
  invoice — except in a sentence that denies it, because banning the words outright makes
  the rule itself unsayable.

  The naming is itself a fix. The document, the changelog and the validation record all
  said every rule was driven over a row corrupted to break it, and an independent review
  found that one of the ten — the completeness check, the rule the register rests on —
  had no such control at all. Each rule now names its control in the data, a test refuses
  a name that is not a function in the suite, and no two rules may share one.
  ...
  **The completeness check is against the README.** Every relative link in its public
  ...
  it says it says is a reading, and no test performs one. That is not a theoretical gap:
  an independent reading of all forty-nine cited records against the rows citing them
  corrected **twenty-two of the fifty-eight rows**, fourteen of them stating the
  repository's state today while citing a record that froze an earlier one — the security control counts, the ownership row counts, the
  gate counts, the template counts, the strategy-drift scope, and a claim that a release
  had been installed among them — plus four misreadings and four that could not be fixed
  by re-citing at all, including a row citing a runtime-packaging record for a claim
  about the API's routes and a `C2` claim resting on a record that calls itself
  `local-static`. All were corrected before
  merge, by citing the later record as well or by restating the figure the cited record
  actually holds, and the matrix now carries the whole episode in its own limitations.
