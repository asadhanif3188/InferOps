...
REQUIRED_LOCK_PLATFORMS = 5

RESOURCE_BLOCK = re.compile(
    r'^resource\s+"(?P<type>[\w-]+)"\s+"(?P<name>[\w-]+)"\s*\{', re.MULTILINE
)
PROVIDER_BLOCK = re.compile(r'^provider\s+"(?P<name>[\w-]+)"\s*\{', re.MULTILINE)
REQUIRED_PROVIDER_ENTRY = re.compile(
    r"^\s{4,}(?P<name>[\w-]+)\s*=\s*\{\s*$", re.MULTILINE
)
...
def declared_resource_types() -> set[str]:
    return {
        match.group("type") for match in RESOURCE_BLOCK.finditer(ALL_TERRAFORM_CODE)
    }
...
def test_no_provider_reaches_beyond_a_local_cluster(provider: str) -> None:
    configured = {
        match.group("name") for match in PROVIDER_BLOCK.finditer(ALL_TERRAFORM_CODE)
    }
    required = {
        match.group("name")
        for match in REQUIRED_PROVIDER_ENTRY.finditer(ALL_TERRAFORM_CODE)
    }
    assert provider not in configured, provider
    assert provider not in required, provider
...
def test_the_only_provider_is_the_kubernetes_one() -> None:
    ...
    assert configured == {"kubernetes"}, sorted(configured)
