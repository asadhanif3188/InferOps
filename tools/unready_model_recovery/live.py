"""Asking the release's own collector what it saw while the model was not ready.
...
Every expression in the descriptor is asked, including the one registered as having
nothing to emit and the one registered as having no source. Asking them is the point:
the metric whose declared question is *exactly* this experiment's question --
`inferops_model_ready` -- is the one the telemetry catalog marks not emitted, and a
record that left it out would be a record of the signals that happened to work.
"""
...
from .core import Descriptor, UnreadyError
...
def _range_result(response: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("resultType") != "matrix":
        raise UnreadyError("a range query did not answer with a matrix")
    result: list[dict[str, Any]] = []
    elements = data.get("result") or []
    if not isinstance(elements, list):
        raise UnreadyError("a range query answered with a result that is not a list")
...
def capture_telemetry(
    ...
    *,
    ...
    exactly at the start of the unready window would otherwise have nothing before it
