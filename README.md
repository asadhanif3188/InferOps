...
| V1 evidence index | [docs/proof/v1-evidence-index.md](docs/proof/v1-evidence-index.md) | One entry per evidence record in the claim and evidence register — what executed, what was substituted, the workload source, the environment and provider, every immutable identifier the record pins, the repository revision it names and whether that is the tree that ran, and every cited file bound to its content by SHA-256. Generated by `python -m tools.evidence_index` from the register and the normalization ledger, and regenerated by a test, so it cannot state what they do not. It shows that 13 of the 31 records that executed their target behaviour name the revision that ran |
