...
To read these results as one narrative, start with
[the V1 engineering case study](docs/case-study/v1-engineering-case-study.md). It
takes five of the rows above as problems investigated — declared load, a lost serving
pod, a model that never finishes loading, the whole path from a clean clone, and the
cost of measured use — and gives what was measured, why it matters to an operator, and
the limitation beside each, before the architecture, the decisions, and what V1 does
...
4. **Check the proof.** [The case study](docs/case-study/v1-engineering-case-study.md)
   reads the results as one narrative; [the results](#results) link the dashboard,
   load, failure, and cost records; [the proof dashboard](docs/proof/dashboard.md)
   indexes the rest, down to each record and the decision records behind it.
