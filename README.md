...
| Test and CI strategy | [docs/testing/test-strategy.md](docs/testing/test-strategy.md) | Strategy accepted and machine-checked; nine of eleven test layers exist and two do not. One lane of four is automated since [ADR 0012](docs/architecture/decisions/ADR-0012-continuous-integration-service.md) and no run of the committed workflow has passed on the service yet |
| Continuous-integration gate matrix | [docs/testing/ci-gate-matrix.md](docs/testing/ci-gate-matrix.md) | Nine gates for the `default-checks` lane, each mapped to the claims it defends or to a recorded reason it defends none, compared to the committed workflow in both directions. The normal lane cannot reach a cluster or download the model, and neither is a promise: both are checked |
